"""Sign-in log sub-orchestrator and page-fetch activity."""
...
from utils.event_parser import parse_sign_in_event
...
def orchestrate_sign_in_logs(context: df.DurableOrchestrationContext):
    ...
    while True:
        context.set_custom_status({
            "step": "fetching_sign_in_logs",
            "message": f"Fetching sign-in logs — page {page + 1} ({total} events so far)...",
            "page": page + 1,
            "count": total,
        })
        ...
        try:
            result = yield context.call_activity_with_retry(
                "fetch_sign_in_log_page_activity", RETRY_OPTIONS, page_input,
            )
        except Exception as exc:
            context.set_custom_status({
                "step": "failed",
                "message": f"Failed fetching sign-in logs on page {page + 1}: {exc}",
                "count": total,
            })
        ...
        if result.get("error"):
            context.set_custom_status({
                "step": "failed",
                "message": f"Sign-in logs fetch failed: {result['error']}",
                "count": total,
            })
    ...
    yield context.call_activity("update_scan_phase_activity", {
        **payload, "phase_name": "sign_in_logs", "phase_status": "completed",
        "items_processed": total,
    })

    context.set_custom_status({
        "step": "completed",
        "message": f"Sign-in logs complete: {total} events across {page} pages.",
        "count": total,
    })
...
def fetch_sign_in_log_page_activity(payload: dict) -> dict:
    """Fetch one page of sign-in logs, parse events, store to Cosmos."""
    ...
    logger.info(
        "fetch_sign_in_log_page START | scan=%s | tenant=%s | page=%d",
    ...
    try:
        ...
        logger.error(
            "fetch_sign_in_log_page FAILED (auth) | scan=%s | page=%d | error=%s",
    ...
    for raw in page_items:
        event_dict, actor_id, actor_name = parse_sign_in_event(tenant_id, raw)
    ...
    if events:
        ...
        logger.info(
            "fetch_sign_in_log_page cosmos upsert | scan=%s | page=%d | written=%d/%d",
    ...
    logger.info(
        "fetch_sign_in_log_page DONE | scan=%s | page=%d | items=%d | actors=%d | has_next=%s | elapsed=%.0fms",
